...
from ..config import USDT_CONTRACT, TRON_FULLNODE_URL, USDT_DECIMALS, TRONGRID_API_KEY, TRONGRID_QPS
...
def _get_client() -> Tron:
    ...
    provider = HTTPProvider(endpoint_uri=TRON_FULLNODE_URL or "https://api.trongrid.io",
                            api_key=api_keys, timeout=20.0)
...
async def _retry_with_backoff(coro_func, *args, **kwargs):
    # 针对 401/403/429 做退避重试；最多 4 次：1s / 2s / 4s / 8s 随机抖动
    for attempt in range(4):
        try:
            ...
            if code in (401, 403, 429) or isinstance(e, requests.HTTPError):
                ...
                collect_logger.warning(f"[TronGrid] 调用受限/未授权，重试{attempt+1}/4，{delay:.2f}s 后重试；err={e}")
...
async def usdt_transfer_all(priv_hex: str, from_addr: str, to_addr: str, amount: float) -> str:
    async def _call():
        ...
        def _task():
            ...
            tx = (usdt.functions.transfer(to_addr, amt)
