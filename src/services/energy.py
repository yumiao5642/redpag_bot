...
async def rent_energy(receive_address: str, pay_nums: int = 65000, rent_time: int = 1, order_notes: Optional[str] = None) -> Dict[str, Any]:
    ...
    collect_logger.info(f"⚡ 正在请求租用能量：payNums={pay_nums}, rentTime={rent_time}, addr={receive_address}, notes='{order_notes}'")
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if data.get("code") != 10000:
            raise RuntimeError(f"trongas 下单失败：code={data.get('code')} msg={data.get('msg')}")
