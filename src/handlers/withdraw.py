...
async def wdpwd_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if key.isdigit() and len(key) == 1:
        ...
        if len(st["buf"]) == 4:
            ...
            # 以下逻辑直接复用 on_text 里原提现执行步骤（去掉输入解析）
...
async def withdraw_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "withdraw_addr_add_start":
        ...
        withdraw_logger.info("💸 新增地址流程开始：用户=%s", u.id)
        return
    ...
    if data.startswith("withdraw_to:"):
        ...
        withdraw_logger.info("💸 选择提现地址：用户=%s，地址ID=%s，别名=%s", u.id, addr_id, target['alias'])
        return
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("wd_wait_amount"):
        ...
        await update.message.reply_text("🔒 请输入资金密码\n----------------------------\n🔑 ____", reply_markup=_wdpwd_kbd())
